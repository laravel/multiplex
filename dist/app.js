...
export function App({ commandDefs, cwd, initialStreamMode, bufferSize = 2000, streamBufferSize = 10000, timestamps = false, outputRef, procsRef: externalProcsRef, }) {
    ...
    const outputLineCountsRef = useRef(commandDefs.map(() => 1));
    ...
    const spawnProcess = useCallback((cmd, i) => {
        ...
        const proc = spawn(spawnArgs[0], spawnArgs[1], {
            ...
            env: {
                ...process.env,
                FORCE_COLOR: "1",
                COLUMNS: String((stdout?.columns ?? 80) -
                    SIDEBAR_WIDTH -
                    CONTENT_BORDER -
                    CONTENT_PADDING -
                    SCROLLBAR_WIDTH),
    ...
    const restartProcess = useCallback((i) => {
    ...
    if (searchQuery) {
        ...
        const hl = highlightSearch(fullContent, searchQuery, searchInputMode ? -1 : currentMatch);
        matchCount = hl.count;
        ...
        if (!searchInputMode && matchCount > 0 && hl.linePositions.length > 0) {
            const effectiveMatch = ((currentMatch % matchCount) + matchCount) % matchCount;
            const targetLine = hl.linePositions[effectiveMatch] ?? 0;
            const halfWindow = Math.floor(outputHeight / 2);
            const maxStart = Math.max(0, highlightedLines.length - outputHeight);
            const scrollStart = Math.max(0, Math.min(targetLine - halfWindow, maxStart));
            visibleLines = highlightedLines.slice(scrollStart, scrollStart + outputHeight);
        }
        else {
        ...
        if (scrollOffset !== null) {
            visibleLines = displayLines.slice(scrollOffset, scrollOffset + outputHeight);
        }
        else {
    ...
    let footer;
    if (searchInputMode) {
        footer = (_jsxs(Text, { children: [_jsxs(Text, { color: "#e5c07b", bold: true, children: ["/", " "] }), _jsx(Text, { color: "#cccccc", children: searchQuery }), _jsx(Text, { color: "#e5c07b", children: "█" }), searchQuery && matchCount === 0 && (_jsx(Text, { color: "#e06c75", children: " no matches" })), searchQuery && matchCount > 0 && (_jsxs(Text, { color: "#555555", children: [" ", matchCount, " match", matchCount === 1 ? "" : "es"] }))] }));
    }
    else if (searchQuery && matchCount > 0) {
        footer = (_jsxs(Text, { children: [_jsxs(Text, { color: "#e5c07b", bold: true, children: ["[", effectiveMatch + 1, "/", matchCount, "]"] }), _jsxs(Text, { color: "#888888", children: [" ", searchQuery] }), _jsx(Text, { color: "#555555", children: " " }), _jsx(Text, { color: "#888888", bold: true, children: "n" }), _jsx(Text, { color: "#555555", children: " next" }), _jsx(Text, { color: "#555555", children: " " }), _jsx(Text, { color: "#888888", bold: true, children: "N" }), _jsx(Text, { color: "#555555", children: " prev" }), _jsx(Text, { color: "#555555", children: " " }), _jsx(Text, { color: "#888888", bold: true, children: "Esc" }), _jsx(Text, { color: "#555555", children: " clear" })] }));
    }
    else {
        ...
        footer = (_jsxs(Box, { width: "100%", children: [_jsx(Text, { children: bindings.map(([k, desc], i) => (_jsxs(Text, { children: [i > 0 && _jsx(Text, { color: "#555555", children: " " }), _jsx(Text, { color: "#888888", bold: true, children: k }), _jsxs(Text, { color: "#555555", children: [" ", desc] })] }, i))) }), _jsx(Box, { flexGrow: 1 }), hasNewOutput && _jsx(Text, { color: "#e5c07b", children: "\u2193 new output " })] }));
    }
    ...
    let thumbEnd = 0;
    if (showScrollbar) {
        const currentOffset = scrollOffset ?? Math.max(0, totalLines - outputHeight);
    ...
    if (streamMode) {
        return (_jsxs(Box, { flexDirection: "column", height: rows, width: cols, children: [_jsx(Box, { flexDirection: "column", flexGrow: 1, paddingLeft: 1, children: outputContent }), _jsx(Box, { height: 1 }), _jsx(Box, { height: 1, paddingLeft: 1, children: footer })] }));
    }
    const focusedBorder = "#61afef";
    const unfocusedBorder = "#333333";
    return (_jsxs(Box, { flexDirection: "column", height: rows, width: cols, children: [_jsxs(Box, { flexDirection: "row", flexGrow: 1, children: [_jsx(Box, { flexDirection: "column", width: sidebarWidth, borderStyle: "round", borderColor: focus === "sidebar" ? focusedBorder : unfocusedBorder, children: commandDefs.map((cmd, i) => {
                            ...
                            return (_jsxs(Box, { children: [_jsxs(Text, { backgroundColor: bg, color: failed
                                            ? "#ef4444"
                                            : selected
                                                ? "#000000"
                                                : "#555555", dimColor: !failed && !selected, children: [" ", indicator, " "] }), _jsxs(Text, { backgroundColor: bg, color: selected
                                            ? "#000000"
                                            : failed
                                                ? "#ef4444"
                                                : cmd.color, children: [cmd.label, " ".repeat(pad)] })] }, i));
                        }) }), _jsxs(Box, { flexDirection: "column", flexGrow: 1, borderStyle: "round", borderColor: focus === "content" ? focusedBorder : unfocusedBorder, children: [_jsx(Box, { borderColor: focus === "content"
                                    ? focusedBorder
                                    : unfocusedBorder, borderDimColor: true, borderTop: false, borderLeft: false, borderRight: false, borderStyle: "single", paddingX: 1, children: _jsx(Text, { color: "#888888", children: commandDefs[selectedIndex].command.join(" ") }) }), _jsx(Box, { paddingLeft: 1, flexDirection: "column", flexGrow: 1, children: outputContent })] })] }), _jsx(Box, { height: 1, paddingLeft: 1, children: footer })] }));
}
